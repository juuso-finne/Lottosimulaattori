...
void Simuloi_lottokierros(Lottokuponki& kuponki, int& voitot, int& tappiot, int tulostuskynnys, int kierrosnumero){
	// Arpoo kierroksen oikeat voittonumerot ja lisänumeron ja lähettää ne funktiolla, joka tarkistaa kupongin. 
	//Kirjaa kierroksen voitot ja tappiot (välitetään viiteparametreinä, koska niitä ei palauteta).
	...
	} while (voittonumerot[lisanumero]); // Varmistetaan, että arvotaan lisänumero, joka ei ole voittonumeroiden joukossa.
...
bool Paavalikko(int& voitot, int& tappiot, int MAX_RIVIT, int MAX_KIERROKSET, Lottokuponki& kuponki) {
	//Antaa käskyn tulostaa päävalikko ja kutsuu ohjelman eri funktioita käyttäjän valinnan perusteella.
...
void Nayta_ohje() {
	// Tulostaa pelin säännöt ja ohjeet näytölle
	Tyhjenna_ruutu();
...
void Anna_koneen_arpoa_rivit(Lottokuponki& kuponki, int max_rivit) {
	// Täyttää halutun määrän lottorivejä annetulle kupongille. Parametri max_rivit kertoo, montako riviä kupongille vielä mahtuu
	Tyhjenna_ruutu();
...
void Tulosta_kuponki(Lottokuponki& kuponki) {
	//Tulostaa kupongin lottorivit näytölle yksi kerrallaan
	Tyhjenna_ruutu();
...
void Pelaa_lottoa(Lottokuponki& kuponki, int& voitot, int& tappiot, int MAX_KIERROKSET) {
	//Kysyy käyttäjältä kierrosten määrän ja tulostusasetukset
	//Varoittaa käyttäjää tulostamasta jokaista voittoriviä, jos niitä on odotettavissa paljon
